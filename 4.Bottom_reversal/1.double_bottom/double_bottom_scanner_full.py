...
def get_extended_stock_pool(stock_map):
    """扩展股票池: 沪深300 + 中证500 + 创业板(300xxx) + 科创板(688xxx)"""
    hs300 = set(get_hs300_components() or [])
    zz500_result = api_call('index_weight', index_code='000905.SH', fields='con_code')
    zz500 = set(item[0] for item in zz500_result['items']) if zz500_result and 'items' in zz500_result else set()
    # 创业板 + 科创板
    gemb = set(c for c in stock_map if c.startswith('300'))
    kcb = set(c for c in stock_map if c.startswith('688'))
    combined = hs300 | zz500 | gemb | kcb
    return [c for c in combined if c in stock_map], {'hs300': len(hs300), 'zz500': len(zz500), 'gemb': len(gemb), 'kcb': len(kcb)}
...
def main():
    ...
    scan_codes, pool_info = get_extended_stock_pool(stock_map)
    print(f"  股票池: 沪深300({pool_info['hs300']}) + 中证500({pool_info['zz500']}) + 创业板({pool_info['gemb']}) + 科创板({pool_info['kcb']})")
    print(f"  合计扫描 {len(scan_codes)} 只股票")
    ...
    for idx, code in enumerate(scan_codes):
        ...
        try:
            ...
            if not plot_df or len(plot_df) < 60: continue

            # 截取到END_DATE用于形态检测
            scan_df = [d for d in plot_df if d['trade_date'] <= END_DATE]
            if len(scan_df) < 60: continue
            
            patterns = detect_double_bottom(scan_df, window=8)
            name = stock_map.get(code, code)
            
            for p in patterns:
                score, reasons = score_pattern(scan_df, p)
                if score >= 40:
                    all_patterns.append({
    ...
    # 导出CSV (全部符合条件的结果, 文件名体现扫描类型)
    csv_path = os.path.join(output_dir, f'双底扫描_results_{END_DATE}.csv')
    ...
    for i, item in enumerate(top10):
        ...
        print(f"  突破日: {p['break_date']} (距今{len(item['scan_df'])-1-p['break_idx']}天)")
