...
END_DATE = '20260619'           # 扫描截止日期（形态必须在此之前突破）
START_DATE = '20241001'
...
def main():
    ...
    for idx, code in enumerate(scan_codes):
        ...
        try:
            ...
            patterns = detect_ihs(scan_df, window=10)
            name = stock_map.get(code, code)

            for p in patterns:
                score, reasons = score_ihs(scan_df, p)
                if score >= 40:
                    all_patterns.append({
    ...
    csv_path = os.path.join(output_dir, f'头肩底扫描_results_{END_DATE}.csv')
    with open(csv_path, 'w') as f:
        ...
        for i, item in enumerate(all_patterns):
            ...
            neck_now = p['neck2_price'] + n_slope * (len(item['plot_df'])-1-p['neck2_idx'])
