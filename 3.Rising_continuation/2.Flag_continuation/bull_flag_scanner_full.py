...
def main():
    ...
    for idx, code in enumerate(scan_codes):
        ...
        try:
            ...
            for f in flags:
                ...
                if score >= 30:
                    if scan_df[f['breakout_idx']]['trade_date'] < earliest_date:
                        continue
                    all_patterns.append({
                        'code': code,
                        'name': name,
    ...
    csv_path = os.path.join(output_dir, f'旗形中继扫描_results_{END_DATE}.csv')
    ...
    if top10:
        latest_date = top10[0]['plot_df'][-1]['trade_date']
        print(f"\n[数据截止日期: {latest_date}]")
